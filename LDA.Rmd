...
interaction_plot <- ggplot(emm_df, aes(x = time, y = emmean, color = supportive_coping, group = supportive_coping)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Interaction Plot of Time and Supportive Coping on BP",
    x = "Time",
    y = "Estimated Marginal Mean of BP"
  ) +
  theme_minimal()

ggsave("Figure/Emmeans_interaction.png", plot = interaction_plot, width = 720, height = 480, units = "px")
...
png(file = "Figure/normality.png", width = 720, height = 480)
qqnorm(residuals, main="Figure 1a: Q-Q Plot of Residuals to Assess Normality")
...
homoscedasticity_plot <- ggplot(data = data.frame(fitted = fitted_values, abs_residuals = abs(residuals)), aes(x = fitted, y = abs_residuals)) +
  ...
  geom_hline(yintercept = 0, col = "red") +
  labs(
    title = "Figure 1b: Residuals vs. Predicated Plot to Assess Homoscedasticity",
    x = "Fitted values",
    y = "Absolute Residuals"
  )
# Save the homoscedasticity plot
ggsave("Figure/Homoscedascity.png", plot = homoscedasticity_plot, width = 720, height = 480, units = "px")
