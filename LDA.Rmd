...
mod1_BP <- afex::mixed(BP ~ time * supportive_coping + (1|ID), set_data_arg = TRUE, check_contrasts = TRUE, data=BP_long)
summary(mod1_BP)
...
mod4_BP <- afex::mixed(BP ~ supportive_coping + (1|ID), set_data_arg = TRUE, check_contrasts = TRUE, data=BP_long)
...
mod1_emm <- emmeans(mod1, ~ time * supportive_coping)
...
ggplot(data = data.frame(fitted = fitted_values, abs_residuals = abs(residuals)), aes(x = fitted, y = abs_residuals)) +
  ...
  ggtitle("Figure 1b: Residuals vs. Predicated Plot to Assess Homoscedasticity")
